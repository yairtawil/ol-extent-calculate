...
export class OlExtentCalculateComponent extends HTMLElement {
	...
	public polygon1 = new Polygon([]);

	public polygon2 = new Polygon([]);
	...
	fitExtent(map: Map, polygon: Polygon) {
		...
		const [[topLeft, topRight, bottomRight, bottomLeft]] = <any> geojson.geometry.coordinates;
		const [width, height] = map.getSize();

		const rotation = this.getAngle(topLeft, topRight);
		const size = width * Math.cos(rotation) + height * Math.sin(rotation);
		const resolution = extent.getWidth(<ol.Extent> [...topLeft, ...bottomRight]) / size;
		const center = turf.center(geojson.geometry);

		view.setCenter(<[number, number]> center.geometry.coordinates);
